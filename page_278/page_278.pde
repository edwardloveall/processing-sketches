...
Spot[] sps = new Spot[6]; //declare the object [x] times
...
void setup() {
  ...
  randomSeed(0);
  for(int i=0; i<sps.length; i++) { //makes as many spots as specified at top
    sps[i] = new Spot();  
  }
}
...
void draw() {
  ...
  fill(0);
  for(int i=0; i<sps.length; i++) { // updates the display 6 times?
    sps[i].updateDisplay();  
  }
}
...
class Spot {
  ...
  Spot() { // all this does is pick a direction at the start i guess...
    float r = random(1.0);
    if (r < .25) {
      directionX = 1;
      directionY = 1;
  ...
  void updateDisplay() {
    ...
    ellipse(x, y, radius, radius); // make a circle
    x += speedX * directionX; 
